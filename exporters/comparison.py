...
def get_quarter_label(d=None):
    """날짜 기준 'n년 Qx' 문자열 반환."""
    if d is None:
        d = date.today()
    quarter = (d.month - 1) // 3 + 1
    return str(d.year) + "년 Q" + str(quarter)
...
def get_month_label(d=None):
    ...
    if d is None:
        d = date.today()
    return str(d.year) + "년 " + str(d.month) + "월"


def get_period_label(period_key, d=None):
    """
    period_key: 'daily' | 'weekly' | 'monthly'
    daily   → 'n년 x월 y주차' (주 단위 갱신)
    weekly  → 'n년 x월'        (월 단위 갱신)
    monthly → 'n년 Qx'         (분기 단위 갱신)
    """
    if d is None:
        d = date.today()
    ...
    if period_key == "daily":
        ...
        if d < first_monday:
            ...
            week_num = (d - first_monday).days // 7 + 2
        return str(d.year) + "년 " + str(month) + "월 " + str(week_num) + "주차"

    elif period_key == "weekly":
        return get_month_label(d)

    elif period_key == "monthly":
        return get_quarter_label(d)

    return get_week_label(d)


def get_naver_series_comparison_filename(period_ko, period_key, category_ko):
    """
    네이버 시리즈 전용 비교 파일명 생성.
    예: '일간 판타지 비교 (2026년 6월 1주차).html'
    """
    label = get_period_label(period_key, date.today())
    return period_ko + " " + category_ko + " 비교 (" + label + ").html"
...
def _get_chartjs():
    """chart.js 스크립트를 CDN에서 가져와 인라인으로 반환. 실패하면 CDN 태그 반환."""
    try:
        import urllib.request
        url = "https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"
        with urllib.request.urlopen(url, timeout=10) as resp:
            return "<script>" + resp.read().decode("utf-8") + "</script>"
    except Exception:
